...
We provide a sample file [here](Desktop/cpgUV/gradle.properties.example) - simply copy it to `gradle.properties` in the directory of the cpg-project.
...
In order to get familiar with the graph itself, you can use the subproject [cpg-neo4j](Desktop/cpgUV/cpg-neo4j). It uses this library to generate the CPG for a set of user-provided code files. The graph is then persisted to a [Neo4j](https://neo4j.com/) graph database. The advantage this has for the user, is that Neo4j's visualization software [Neo4j Browser](https://neo4j.com/developer/neo4j-browser/) can be used to graphically look at the CPG nodes and edges, instead of their Java representations.
...
The library can be used on the command line using the `cpg-console` subproject. Please refer to the [README.md](Desktop/cpgUV/cpg-console/README.md) of the `cpg-console` as well as our small [tutorial](Desktop/cpgUV/tutorial.md) for further details.
...
We provide a sample file with all languages switched on [here](Desktop/cpgUV/gradle.properties.example).
...
You need to install [jep](https://github.com/ninia/jep/). This can either be system-wide or in a virtual environment. Your jep version has to match the version used by the CPG (see [version catalog](Desktop/cpgUV/gradle/libs.versions.toml)).
