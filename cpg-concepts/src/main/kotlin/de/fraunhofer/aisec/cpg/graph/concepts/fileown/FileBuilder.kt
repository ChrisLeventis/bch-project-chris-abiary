...
fun MetadataProvider.newFileHandler(underlyingNode: Node): FileHandler {
    val node = FileHandler(underlyingNode = underlyingNode)
    node.codeAndLocationFrom(underlyingNode)
    node.name = Name("FileHandler[" + underlyingNode.name.toString() + "]")
    NodeBuilder.log(node)
    return node
}

fun MetadataProvider.newFileOpWrite(
    underlyingNode: Node,
    handler: FileHandler,
    what: Node?,
): FileOpWrite {
