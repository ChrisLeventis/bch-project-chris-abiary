...
fun MetadataProvider.newWebsocketClientNode(underlyingNode: Node): WebsocketClient {
    val node = WebsocketClient(underlyingNode = underlyingNode)
    node.codeAndLocationFrom(underlyingNode)
    node.name = Name("WebsocketClient[" + underlyingNode.name.toString() + "]")
    NodeBuilder.log(node)
    return node
}

fun MetadataProvider.newWebsocketOpSendNode(
    underlyingNode: Node,
